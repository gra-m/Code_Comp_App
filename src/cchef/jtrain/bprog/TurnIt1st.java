...
public static void main(String[] args) throws Exception {
	...
	int caseLength = 4;
      ...
      while( t-- > 0 ) {
            ...
            double v_limit = Integer.parseInt(arr[1]);
            ...
            if( kase > 0 ) {
			OUT.println(u_mps + " " + v_limit + " " + a_decel + " " + s_disToTurn);
                  double velocitySquared =
                      (u_mps * u_mps) + (2 * a_decel * s_disToTurn);
                  OUT.println("( velocity = u_mps " + u_mps + " * " + "u_mps "
                      + u_mps + ")" + " + " + "( 2 * " + "accel/decel rate per sec " +
                      "sq" + " " + a_decel + " * " + "s_disToTurn " +
                      s_disToTurn + " ) = velocitySquared " + velocitySquared);
                  double negativeVelocitySquared =
                      Math.sqrt((u_mps * u_mps) - (2 * a_decel * s_disToTurn));
                  OUT.println("negative velocity " +
                      "squared = " + negativeVelocitySquared);
                  OUT.println("velocity limit for turn is: " + v_limit);
                  OUT.println("----");
                  if (u_mps <= v_limit)
                        OUT.println("YES");
                  else if (negativeVelocitySquared <= velocitySquared)
                        OUT.println("YES");
                  else
                        OUT.println("NO");
