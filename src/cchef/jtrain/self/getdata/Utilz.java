...
public final class Utilz {
  ...
  /**
   * Gets case length and confirms file validity in that the quoted cases are
   * divisible into total data lines in file (array length - 1).
   */
 ...
 * @throws IOException
 */
public static void createTestInput(int cases, int caseLength, boolean countAtStart) throws IOException {
        int count = cases;
        if (countAtStart)
              OUT.println(cases);
          while (cases-- > 0) {
