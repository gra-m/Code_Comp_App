...
public static void main(String[] args) throws Exception {
	...
	while (t-- > 0) {
		...
		String numAsString = arr[0];
		double fractionalPart = 0;
		int wholeNumPart;
		StringBuilder res = new StringBuilder();
		int binaryFractionSteps = 100;
		final int STEPS = binaryFractionSteps;

		int pointIndex = numAsString.indexOf('.');

		// check if whole already
		if (pointIndex != -1) {
			fractionalPart = Double.parseDouble(numAsString.substring(pointIndex));
			wholeNumPart = Integer.parseInt(numAsString.substring(0, pointIndex));
		} else {
			wholeNumPart = Integer.parseInt(numAsString);
		}
		

		//OUT.println(fractionalPart);
		
			while( wholeNumPart != 0 ) {
				//OUT.println("test" + " " + wholeNumPart);
				res.insert(0,wholeNumPart%2);
				wholeNumPart = wholeNumPart/2;
			}

			while ( fractionalPart != 0 && binaryFractionSteps-- != 0) {
				//OUT.println(fractionalPart + " fract currently");
				if (binaryFractionSteps == STEPS - 1) res.append('.');
				fractionalPart = fractionalPart * 2;
				res.append(fractionalPart >= 1 ? 1 : 0);
