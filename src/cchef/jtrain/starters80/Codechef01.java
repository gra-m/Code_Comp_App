package cchef.jtrain.starters80; // running comparison
...
public static void main(String[] args) throws Exception {
	...
	while (t-- > 0) {
		int n = s.nextInt();
		int[] aliceArr = s.readIntArr(n);
		int[] bobArr = s.readIntArr(n);
		boolean AliceHappy = false;
		boolean BobHappy = false;
		int happyCount = 0;

		outeri:
		for (int i = 0; i < n; i++) {
			int alice = aliceArr[i];
			int bob = bobArr[i];
